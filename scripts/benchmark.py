...
class LLMBenchmark:
    ...
    def _create_detailed_charts(self, df, timestamp):
        ...
        model_efficiency = df.groupby('model_name').apply(
            lambda x: x['quality_score'].mean() / (x['cost_estimate'].mean() * 1000) if x['cost_estimate'].mean() > 0 else x['quality_score'].mean() * 1000
...
async def main():
    ...
    if api_keys.get('openrouter'):
        try:
            benchmark.add_model(QwenInterface(api_keys['openrouter'], "qwen/qwen3-32b:free"))
            print("✓ Added Qwen3 32B (via OpenRouter)")
            models_added += 1
        except Exception as e:
